    ...
    "; 
        ...
        ";
            ...
            ";
                    ...
                    {
                        // Aynı anda başka istek açtı -> tekrar mevcut açık siparişi oku  
...
";
        ...
        ";
                ...
                // 2) Ürünleri FOR UPDATE ile lockla
        ...
        ";
                ...
                // 3) Validasyon: ürün var mı + adet valid mi
                ...
                // 4) Aynı ürünler toplansın
                var grouped = items
                    .GroupBy(x => x.UrunId)
                    .Select(g => new { UrunId = g.Key, Adet = g.Sum(x => x.Adet) })
                    .ToList();

                // 5) Stok yeterli mi? (ön kontrol)
                ...
                // 6) Kalem UPSERT (satir_toplam'ı DB hesaplasın) // *
    ...
    int siparisId, decimal iskontoOran, string? actorUsername) 
        ...
        ";
                ...
                var items = new List<RestaurantWeb.Models.SiparisDetayDto>();
            ...
            int siparisId, OdemeYontemi yontem, string? actorUsername, int kapatanPersonelId)
        {
            ...
            // Enum defensive check (controller bypass edilebilir)  
            if (!Enum.IsDefined(typeof(OdemeYontemi), yontem)) 
                return OperationResult.Fail("Geçersiz ödeme yöntemi."); 

            try
            {
                // Siparişi kilitle + durumunu da oku (idempotency için)  
                const string getSql = @"
        ...
        "; 
                ...
                // Zaten kapalıysa idempotent OK dön  
                if (durum != 0) 
                    return OperationResult.Ok("Sipariş zaten kapalı/ödenmiş."); 

                // Ödeme kaydı (unique varsa ikinci denemede 23505)  
                const string paySql = @"
        ...
        ";
                ...
                // Siparişi kapat (idempotent UPDATE: sadece açıksa)  
